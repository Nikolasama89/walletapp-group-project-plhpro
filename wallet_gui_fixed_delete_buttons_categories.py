...
class WalletApp(tk.Tk):
    ...
    def build_transactions_tab(self):
        ...
        ttk.Combobox(form, textvariable=self.txn_type_var, values=["income", "expense"], state="readonly", width=15).grid(row=0, column=1, padx=5, pady=5)
        ...
        self.txn_category_combo = ttk.Combobox(form, textvariable=self.txn_category_var, state="readonly", width=25)
        ...
        ttk.Button(form, text="Add Transaction", command=self.add_transaction).grid(row=1, column=5, padx=5, pady=5)

        self.txn_type_var.trace_add("write", lambda *_: self.refresh_category_dropdowns())

        txn_button_frame = ttk.Frame(self.transactions_tab)
        txn_button_frame.pack(fill="x", padx=10, pady=(0, 5))
    ...
    def delete_selected_transaction(self):
        ...
        if not selected:
            messagebox.showinfo("Info", "Select a transaction from the table first.")
            return

        txn_id = self.transactions_tree.item(selected[0])["values"][0]
        confirm = messagebox.askyesno("Confirm Delete", f"Delete transaction with ID {txn_id}?")
        if not confirm:
            return
    ...
    def build_tasks_tab(self):
        ...
        ttk.Combobox(form, textvariable=self.task_type_var, values=["obligation", "wish"], state="readonly", width=15).grid(row=0, column=1, padx=5, pady=5)
        ...
        ttk.Combobox(form, textvariable=self.task_priority_var, values=["1", "2", "3"], state="readonly", width=8).grid(row=1, column=3, padx=5, pady=5, sticky="w")

        ttk.Button(form, text="Add Task", command=self.add_task).grid(row=1, column=5, padx=5, pady=5)

        task_button_frame = ttk.Frame(self.tasks_tab)
        task_button_frame.pack(fill="x", padx=10, pady=(0, 5))

        ttk.Button(task_button_frame, text="Mark Selected Done", command=self.mark_task_done).pack(side="left", padx=5)
        ttk.Button(task_button_frame, text="Delete Selected Task", command=self.delete_selected_task).pack(side="left", padx=5)
    ...
    def mark_task_done(self):
        ...
        if not selected:
            messagebox.showinfo("Info", "Select a task from the table first.")
            return
    ...
    def delete_selected_task(self):
        ...
        task_id = self.tasks_tree.item(selected[0])["values"][0]
        confirm = messagebox.askyesno("Confirm Delete", f"Delete task with ID {task_id}?")
        if not confirm:
            return
    ...
    def build_categories_tab(self):
        ...
        ttk.Combobox(form, textvariable=self.category_type_var, values=["income", "expense", "both"], state="readonly", width=15).grid(row=0, column=3, padx=5, pady=5)
        ...
        ttk.Button(form, text="Add Category", command=self.add_category).grid(row=0, column=6, padx=5, pady=5)

        category_button_frame = ttk.Frame(self.categories_tab)
        category_button_frame.pack(fill="x", padx=10, pady=(0, 5))
    ...
    def delete_selected_category(self):
        ...
        if not selected:
            messagebox.showinfo("Info", "Select a category from the table first.")
            return
        ...
        confirm = messagebox.askyesno(
            "Confirm Delete",
            f"Delete/archive category with ID {category_id}?"
        )
        ...
        try:
            # hard=False means soft delete / archive.
            # This is safer because old transactions can still keep their history.
            self.category_service.delete_category(category_id, hard=False)
            self.refresh_all()
            messagebox.showinfo("Deleted", "Category archived successfully.")
        except Exception as exc:
            messagebox.showerror("Error", str(exc))
    ...
    def refresh_categories(self):
        ...
        for cat in self.categories:
            self.categories_tree.insert(
                "",
                "end",
                values=(cat.id, cat.name, cat.type, cat.monthly_budget if cat.monthly_budget is not None else "", "yes" if cat.is_active else "no"),
            )
    ...
    def refresh_transactions(self):
        ...
        for txn in txns:
            self.transactions_tree.insert("", "end", values=(txn.id, txn.txn_date, txn.txn_type, txn.amount, txn.category_name or "", txn.note or ""))

    def refresh_tasks(self):
        self.tasks_tree.delete(*self.tasks_tree.get_children())
        tasks = self.task_service.list_tasks()
        for task in tasks:
            self.tasks_tree.insert("", "end", values=(task.id, task.task_type, task.title, task.amount or "", task.due_date or "", task.priority, task.status))
    ...
    def draw_expense_pie_chart(self, year, month):
        rows = self.stats_service.get_category_distribution("expense", year=year, month=month)
