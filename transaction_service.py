...
class TransactionService:
    ...
    def validate_date(self, txn_date):
        """Δέχεται ημερομηνία σε μορφή YYYY-MM-DD."""
        if txn_date is None:
        ...
        raise ValidationError("Μη έγκυρη ημερομηνία")
    ...
    def validate_category(self, category_id, txn_type):
        ...
        if not category.is_active:
            raise ValidationError(f"Η κατηγορία {category.name} είναι ανενεργή.")

        if category.type != "both" and category.type != txn_type:
            raise ValidationError(
                f"Η κατηγορία {category.name} δεν επιτρέπεται για "
                f"{'έσοδα' if txn_type == 'income' else 'έξοδα'}."
            )

        return category_id

    def add_income(self, amount, category_id=None, txn_date=None, note=None):
        return self.create_transaction("income", amount, category_id, txn_date, note)

    def add_expense(self, amount, category_id=None, txn_date=None, note=None):
        return self.create_transaction("expense", amount, category_id, txn_date, note)
    ...
    def get_transaction(self, txn_id):
        ...
        if txn is None:
            raise NotFoundError(f"Δεν βρέθηκε συναλλαγή με id={txn_id}")
        return txn
    ...
    def delete_transaction(self, txn_id):
        """
        Διαγράφει μία συναλλαγή.

        Πρώτα γίνεται έλεγχος ότι υπάρχει η συναλλαγή.
